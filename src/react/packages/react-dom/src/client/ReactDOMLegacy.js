...
function legacyCreateRootFromDOMContainer(
  ...
  if (!shouldHydrate) {
    let warned = false;
    let rootSibling;
    // rootSibling 将要存储 container 内部的子元素
    // 开启循环 删除 container 容器中的节点
    // container.lastChild 获取 container 最后的子元素，并且把子元素赋值给rootSibling
...
function legacyRenderSubtreeIntoContainer(
  ...
  if (!root) {
    ...
    // legacyCreateRootFromDOMContainer 这方法就是在创建 FiberRoot 和 rootFiber
    // createLegacyRoot
    ...
    if (isModernRoot) {
      console.error(
        'You are calling ReactDOM.hydrate() on a container that was previously ' +
          'passed to ReactDOM.createRoot(). This is not suppor ted. ' +
 ...
 * container 渲染容器，就是指 id 为 root 的 div ，也就是那个 dom 对象
 * callback 渲染完成后执行的回调函数 可选参数
 */
