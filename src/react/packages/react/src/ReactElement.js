...
function defineKeyPropWarningGetter(props, displayName) {
  ...
  const warnAboutAccessingKey = function () {
    // 在开发环境中
    if (__DEV__) {
      // specialPropKeyWarningShown 是全局变量，不刷新只报一次
...
const ReactElement = function (type, key, ref, self, source, owner, props) {
  const element = {
     ...
     * REACT_ELEMENT_TYPE 就是判断浏览器是否支持 Symbol 值，如果支持就用 Symbol 值，如果不用就用十六进制数值（代表组件类型）
  ...
  // 返回 ReactElement， 此时的 element 就是最终 jsx 转换后的 React元素
  return element;
};
...
/**
 * 创建 React Element
 * type      元素类型（div, span, 或者app组件）
 * config    配置属性（元素的props属性, 还有特殊的属性，例如react源码中的ref属性, key属性），如果config有属性就是对象，没属性就是null
 * children  子元素 (当前元素的子元素)
 *
 * 1. 分离 props 属性和特殊属性
...
export function createElement(type, config, children) {
  ...
  // 如果 config 不为 null，分离普通属性和特殊属性
  if (config != null) {
    // 如果 config 对象中有合法的 ref 属性
    // hasValidRef 判断是否有 ref 属性
    if (hasValidRef(config)) {
    ...
    for (propName in config) {
      ...
      // RESERVED_PROPS 里面存储的就是所有的特殊属性
      // 此判断获取的就是普通属性
      if (
        // hasOwnProperty.call(config, propName)， 判断 config 中是否有 propName 属性，并且在 RESERVED_PROPS 对象中不存在该属性
        hasOwnProperty.call(config, propName) &&
        !RESERVED_PROPS.hasOwnProperty(propName)
      ) {
  ...
  // arguments 是实参集合， arguments.length - 2 就是把传递过来的 type, config 2个参数去掉。看有多少个子元素
  ...
  if (childrenLength === 1) {
    ...
    for (let i = 0; i < childrenLength; i++) {
      ...
      // i + 2 的原因是实参集合的前两个参数不是子元素，所以直接区到了 children 这个实参
  ...
  if (__DEV__) {
    ...
    if (key || ref) {
      ...
      // 如果是 function 说明是个组件
      // type.displayName 的属性
      // type.name 组件函数的名字
      // 如果不是 function 那就返回 type, 不是字符串应该就是 div
      const displayName =
        typeof type === 'function'
          ? type.displayName || type.name || 'Unknown'
          : type;
...
export function isValidElement(object) {
  //在 isValidElement方法中定义了一个常量， REACT_ELEMENT_TYPE。这个常量是在 shared/ReactSymbols 文件中定义的。里面主要是 判断浏览器是否支持 Symbol ，如果支持 Symbol 就是 Symbol值，如果不支持就是十六进制的值
  return (
